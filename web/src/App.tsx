...
export default function App() {
	...
	const saveConfigToBackend = useCallback(
		async (updatedConfig: YamlInfrastructureConfig) => {
	...
	const handleConfigChange = useCallback(
		async (updates: Partial<YamlInfrastructureConfig>) => {
	...
	const handleAddCloudFront = async (
		distribution: NonNullable<
			YamlInfrastructureConfig["cloudfront_distributions"]
		>[0],
	) => {
		if (!config) return;
	...
	const handleDeleteNode = async (nodeId: string, nodeType: string) => {
		...
		if (nodeType === "service") {
			...
			updatedConfig.cloudfront_distributions = (
				config.cloudfront_distributions || []
			).filter((d) => d.name !== distName);
		}
		...
		<PricingProvider>
          ...
          `}
						...
						<div className="flex-1 w-full relative overflow-hidden flex flex-col">
							...
							<div className="flex-1 w-full h-full absolute inset-0 z-10 bg-gray-950">
								<ReactFlowProvider>
									<DeploymentCanvas
										...
										onAddScheduledTask={() =>
											setShowAddScheduledTaskDialog(true)
										}
							...
							<div
								className={`flex-1 w-full h-full absolute inset-0 bg-gray-950 transition-opacity duration-300 ${viewMode === "code" ? "z-50 opacity-100" : "z-0 opacity-0 pointer-events-none"}`}
							>
