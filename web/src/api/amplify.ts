...
export const amplifyApi = {
   ...
   */
  async getApps(environment: string, profile?: string): Promise<AmplifyAppsResponse> {
  ...
  async getBuildLogs(
    ...
    const response = await fetch(`${API_BASE_URL}/amplify/build-logs?${params}`);
    if (!response.ok) {
      const error = await response.text();
   ...
   */
  async triggerBuild(request: TriggerBuildRequest): Promise<TriggerBuildResponse> {
