...
export class SSHWebSocketManager {
	...
	getConnection(key: string, config: ConnectionConfig): WebSocket {
		...
		if (
			...
			console.log(
				`Reusing existing connection for key: ${key}, state: ${existing.readyState}`,
			);
			// Update callbacks
		...
		console.log(
			`Creating new WebSocket connection for key: ${key}, URL: ${config.url}`,
		);
		...
		ws.onclose = (event) => {
			console.log(
				`SSH WebSocket closed for key ${key}, code: ${event.code}, reason: ${event.reason}`,
			);
		...
		(ws as WebSocket & { sendInput?: (input: string) => void }).sendInput = (
			...
			if (ws.readyState === WebSocket.OPEN) {
				...
				console.warn(
					"WebSocket not open, cannot send input. State:",
					ws.readyState,
				);
			}
		};
