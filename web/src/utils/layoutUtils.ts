...
export function preventNodeOverlap(nodes: Node[], margin: number = MINIMUM_MARGIN): Node[] {
    ...
    while (hasOverlap && iterations < maxIterations) {
      ...
      for (const processed of processedBounds) {
        ...
        if (overlap) {
          ...
          const adjustment = calculateAdjustment(currentBounds, processed, margin);
...
export function layoutNodesWithGroups(nodes: Node[], margin: number = MINIMUM_MARGIN): Node[] {
  ...
  const adjustedRegularNodes = regularNodes.map(node => {
    ...
    if (nodeGroup) {
      ...
      const group = adjustedGroups.find(g => g.data?.label?.includes(nodeGroup));
      if (group && group.style) {
        ...
        const maxX = groupX + groupWidth - NODE_DIMENSIONS.service.width - padding;
        const minY = groupY + padding + 40; // Extra space for group label
        const maxY = groupY + groupHeight - NODE_DIMENSIONS.service.height - padding;
