...
export function EventBridgeTestEvent({ config }: EventBridgeTestEventProps) {
  ...
  const allConfiguredSources = [...new Set(configuredEventTasks.flatMap(task => task.sources || []))];
  const allConfiguredDetailTypes = [...new Set(configuredEventTasks.flatMap(task => task.detail_types || []))];
  ...
  return (
    <div className="space-y-4">
      ...
      <Alert>
        ...
        <AlertDescription>
          Send test events to EventBridge to test your event-driven infrastructure. Events will be published 
          to your custom event bus and can trigger any configured event processor tasks.
        </AlertDescription>
      </Alert>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            {allConfiguredSources.length > 0 && (
              <div className="space-y-1">
                ...
                {!allConfiguredSources.includes(testEvent.source) && testEvent.source && (
          ...
          <div className="space-y-2">
            ...
            {allConfiguredDetailTypes.length > 0 && (
              <div className="space-y-1">
                <p className="text-xs text-gray-500">
                  Event processor detail types: {allConfiguredDetailTypes.join(', ')}
                </p>
                {!allConfiguredDetailTypes.includes(testEvent.detailType) && testEvent.detailType && (
                  <p className="text-xs text-yellow-500 flex items-center gap-1">
                    <Info className="w-3 h-3" />
                    This detail type doesn't match any configured event processors
                  </p>
                )}
              </div>
            )}
            ...
            <div className="space-y-3">
              {configuredEventTasks.map((task, index) => (
                <div key={index} className="p-3 bg-gray-800 rounded-lg">
                  <h4 className="text-sm font-medium text-gray-200 mb-2">{task.name}</h4>
                  <div className="space-y-1 text-xs text-gray-400">
                    <div>
                      <span className="text-blue-400">Sources:</span> {task.sources?.join(', ') || 'None'}
                    </div>
                    <div>
                      <span className="text-blue-400">Detail Types:</span> {task.detail_types?.join(', ') || 'None'}
                    </div>
                    <div>
                      <span className="text-blue-400">Rule:</span> {task.rule_name}
                    </div>
                  </div>
                </div>
              ))}
  ...
  "message": "This is a test event from EventBridge"
            ...
            <div className="p-3 bg-gray-800 rounded-lg">
              <h4 className="text-sm font-medium text-gray-300 mb-2">Application Event</h4>
              <pre className="text-xs text-gray-400 overflow-x-auto">{`{
  ...
  "source": "user-service"
            ...
            <div className="p-3 bg-gray-800 rounded-lg">
              <h4 className="text-sm font-medium text-gray-300 mb-2">System Event</h4>
              <pre className="text-xs text-gray-400 overflow-x-auto">{`{
  ...
  "timestamp": "${new Date().toISOString()}"
            ...
            <div className="p-3 bg-gray-800 rounded-lg">
              <h4 className="text-sm font-medium text-gray-300 mb-2">Business Event</h4>
              <pre className="text-xs text-gray-400 overflow-x-auto">{`{
        ...
        <CardContent>
          <ul className="text-sm text-gray-300 space-y-2">
            <li className="flex items-start gap-2">
              ...
              <span>Use <code className="text-blue-400 text-xs">meroku.test</code> as the source for test events to easily identify them</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-blue-400">•</span>
            ...
            <li className="flex items-start gap-2">
              <span className="text-blue-400">•</span>
              <span>Check CloudWatch Logs for event processor tasks to see if they were triggered</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-blue-400">•</span>
              <span>Events are published to your custom event bus: <code className="text-blue-400 text-xs">{config.project}_{config.env}_eventbus</code></span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-blue-400">•</span>
              <span>Use EventBridge console to monitor event delivery and rule matches</span>
            </li>
          </ul>
        </CardContent>
      </Card>
