...
import { AlertCircle, Check, Loader2, Github, ExternalLink, Copy, Plus, Trash2 } from 'lucide-react';
...
export const AddAmplifyDialog: React.FC<AddAmplifyDialogProps> = ({
  ...
  const getSsmParameterPath = () => {
    return `/${projectName}/${environmentName}/github/amplify-token`;
  };
  ...
  const checkExistingToken = async () => {
    ...
    try {
      ...
      const parameter = await infrastructureApi.getSSMParameter(parameterPath);
      ...
      if (parameter && parameter.value) {
        ...
        setFormData(prev => ({
          ...prev,
          github_oauth_token: parameterPath,
        }));
      }
    } catch (error) {
  ...
  const startGitHubDeviceFlow = async () => {
    ...
    try {
      ...
      const interval = setInterval(async () => {
        try {
          const status = await infrastructureApi.checkGitHubAuthStatus(data.user_code);
          ...
          if (status.status === 'authorized') {
            ...
            const ssmPath = getSsmParameterPath();
            setFormData({
              ...formData,
      ...
      }, (data.interval || 5) * 1000); // Poll at the interval specified by backend
      ...
      setDeviceFlowInProgress(false);
      setTokenError(error instanceof Error ? error.message : 'Failed to start authentication');
    }
  };
  ...
  const updateBranch = (index: number, field: string, value: string | boolean) => {
    ...
    } else if (field === 'custom_subdomains_text' && typeof value === 'string') {
      // Parse custom subdomains from text (comma-separated)
      const subdomains = value.split(',').map((s: string) => s.trim()).filter((s: string) => s);
      updatedBranches[index] = {
        ...updatedBranches[index],
  ...
  const handleSubmit = () => {
    ...
    if (!formData.name) {
      ...
      newErrors.name = 'App name must contain only lowercase letters, numbers, and hyphens';
    ...
    if (!formData.github_repository) {
      ...
      newErrors.github_repository = 'Repository URL must start with https://github.com/';
    }
    ...
    if (!formData.branches || formData.branches.length === 0) {
      ...
      formData.branches.forEach((branch, index) => {
        if (!branch.name) {
          newErrors[`branch_${index}_name`] = `Branch ${index + 1}: Name is required`;
        } else if (!/^[a-zA-Z0-9-_/]+$/.test(branch.name)) {
          newErrors[`branch_${index}_name`] = `Branch ${index + 1}: Invalid branch name`;
        }
      });
      
      // Custom domain validation
      if (formData.custom_domain && !formData.branches.some(b => b.stage === 'PRODUCTION')) {
        newErrors.custom_domain = 'Custom domain requires at least one PRODUCTION branch';
      }
    }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure AWS Amplify for hosting your frontend application with continuous deployment.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-2">
            ...
            <div>
              ...
              <Input
                ...
                className={`mt-1 ${errors.github_repository ? 'border-red-500' : ''}`}
              />
              {errors.github_repository && <p className="text-xs text-red-500 mt-1">{errors.github_repository}</p>}
            </div>
            ...
            <div>
              ...
              <div className="border rounded-lg p-4 space-y-3 mt-1">
                {!tokenSaved && !userCode ? (
                  <>
                    <p className="text-sm text-gray-600">
                      Authenticate with GitHub to grant Amplify access to your repository
                    </p>
                    ...
                    <div className="space-y-4">
                      <div className="text-center space-y-2">
                        ...
                        <p className="text-xs text-gray-500">
                          Code copied to clipboard! A popup window should have opened with GitHub. If not, click the button below.
                        </p>
                      </div>
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <p className="text-xs text-center text-gray-500">
                          Or visit: <code className="bg-gray-800 text-gray-300 px-1 rounded">{verificationUri}</code>
                        </p>
                      </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-green-600">
                      ...
                      <span className="text-sm font-medium">GitHub authenticated successfully</span>
                    </div>
                    
                    <p className="text-xs text-gray-600">
                      Token stored in SSM: <code className="bg-gray-800 text-gray-300 px-1 rounded">{getSsmParameterPath()}</code>
                    </p>
                ...
                <div className="pt-2 border-t space-y-1">
                  <p className="text-xs text-gray-500">
                    Required scope: <code className="bg-gray-800 text-gray-300 px-1 rounded">repo</code> (for private repository access)
                  </p>
                  <p className="text-xs text-gray-500">
                    Using GitHub Device Flow - no backend or client secret required
                  </p>
                </div>
              </div>
              
              {errors.github_oauth_token && <p className="text-xs text-red-500">{errors.github_oauth_token}</p>}
            </div>
          ...
          <div className="grid gap-2">
            ...
            {errors.branches && <p className="text-xs text-red-500">{errors.branches}</p>}
            ...
            <div className="space-y-3">
              {formData.branches.map((branch, index) => (
                <div key={index} className="border rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="main"
                        className={`mt-1 ${errors[`branch_${index}_name`] ? 'border-red-500' : ''}`}
                      />
                      {errors[`branch_${index}_name`] && (
                        <p className="text-xs text-red-500 mt-1">{errors[`branch_${index}_name`]}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <Select
                        value={branch.stage}
                        onValueChange={(value) => updateBranch(index, 'stage', value)}
                      >
                        <SelectTrigger id={`branch-stage-${index}`} className="mt-1">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => 
                          updateBranch(index, 'enable_auto_build', checked)
                        }
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => 
                          updateBranch(index, 'enable_pull_request_preview', checked)
                        }
                      />
                  ...
                  <div>
                    <Label htmlFor={`env-vars-${index}`}>Environment Variables</Label>
                    <Textarea
                      id={`env-vars-${index}`}
                      value={branch.environment_variables_text || ''}
                      onChange={(e) => updateBranch(index, 'environment_variables_text', e.target.value)}
                      placeholder="REACT_APP_API_URL=https://api.example.com&#10;REACT_APP_ENV=production"
                      rows={3}
                      className="mt-1"
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Enter one per line in KEY=VALUE format. Use ${'{'}variable{'}'} for interpolation.
                    </p>
                  </div>
          ...
          <div className="bg-blue-950/30 rounded-lg p-3 border border-blue-900/50">
            <p className="text-sm text-blue-200">
              <strong>Amplify Auto-Detection:</strong> Amplify will automatically detect your framework and build settings.
            </p>
            <p className="text-xs text-blue-100 mt-2">
              For custom builds, create an <code className="bg-blue-900/50 px-1 rounded">amplify.yml</code> file in your repository.
            </p>
          </div>

          {/* Domain Configuration */}
          <div className="grid gap-2">
            <h3 className="text-sm font-semibold">Domain Configuration (Optional)</h3>
            ...
            <div>
              ...
              <Input
                ...
                className={`mt-1 ${errors.custom_domain ? 'border-red-500' : ''}`}
              />
              {errors.custom_domain && <p className="text-xs text-red-500 mt-1">{errors.custom_domain}</p>}
              <p className="text-xs text-gray-500 mt-1">Custom domain will be mapped to the first PRODUCTION branch</p>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => 
                  setFormData({ ...formData, enable_root_domain: checked as boolean })
                }
              />
