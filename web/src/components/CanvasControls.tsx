import { Plus, Minus, Maximize, Grid3x3, Move, MousePointer, Eye, EyeOff, Server, Clock, Zap, Globe, Bug } from 'lucide-react';
...
export function CanvasControls({ showInactive, onToggleInactive, onAddService, onAddScheduledTask, onAddEventTask, onAddAmplify }: CanvasControlsProps) {
  ...
  useEffect(() => {
    const storedDebugMode = window.localStorage.getItem('debug_positions') === 'true';
  ...
  const handleDebugPositions = () => {
    ...
    console.log('Verbose position logging is now ' + (newDebugMode ? 'enabled' : 'disabled'));
    console.log('');
    ...
    edges.forEach(edge => {
      ...
      console.log(`  Target: ${edge.target} (handle: ${edge.targetHandle || 'default'})`);
    ...
    console.log('  Right-click on the position object above and select "Copy object"');
    console.log('');
    console.log('To find missing nodes:');
    console.log('  Compare the node IDs above with expected nodes in your configuration');
    console.log('');
    console.log('Debug mode is now:', debugMode ? 'ON' : 'OFF');
    console.log('When ON, all position save/load operations will be logged in detail');
  };
