...
const buildStatusConfig: Record<string, { color: string; icon: any; text: string; pulse?: boolean }> = {
  SUCCEED: { color: 'bg-green-500', icon: CheckCircle, text: 'Success' },
  FAILED: { color: 'bg-red-500', icon: XCircle, text: 'Failed' },
  RUNNING: { color: 'bg-blue-500', icon: Loader2, text: 'Running', pulse: true },
...
export function AmplifyApps({ environment, profile }: AmplifyAppsProps) {
  ...
  const renderBuildStatus = (status: string, isAnimated = false) => {
    const config = buildStatusConfig[status as keyof typeof buildStatusConfig] || buildStatusConfig.PENDING;
    const Icon = config.icon;
    
    return (
      <div className={`flex items-center gap-1.5 ${isAnimated && config.pulse ? 'animate-pulse' : ''}`}>
      ...
      {apps.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground">No Amplify applications found</p>
          </CardContent>
        </Card>
      ) : (
        apps.map((app) => (
                    ...
                    <div className="flex flex-col gap-1">
                      {app.defaultDomain && (
                        <span className="flex items-center gap-1 text-xs">
                          ...
                          <span className="text-gray-400">{app.defaultDomain}</span>
                        </span>
                      )}
                      {app.customDomain && (
                        <span className="flex items-center gap-1 text-xs">
                          ...
                          <span className="text-gray-400">{app.customDomain}</span>
                        </span>
                      )}
                    </div>
                  </CardDescription>
                ...
                {app.branches.map((branch) => (
                  ...
                  >
                    <div className="border rounded-lg p-3">
                      <CollapsibleTrigger
                        onClick={() => toggleAppExpansion(`${app.appId}-${branch.branchName}`)}
                        className="w-full"
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            {branch.stage && (
                              ...
                              >
                                {stageConfig[branch.stage as keyof typeof stageConfig]?.text || branch.stage}
                              </Badge>
                            )}
                          </div>
                          <div className="flex items-center gap-3">
                            {renderBuildStatus(branch.lastBuildStatus || 'PENDING')}
                            {expandedApps.has(`${app.appId}-${branch.branchName}`) ? (
                      ...
                      <CollapsibleContent>
                        <div className="mt-3 pt-3 border-t space-y-3">
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                              ...
                              <p className="font-medium flex items-center gap-1">
                                ...
                                {branch.lastBuildTime ? format(new Date(branch.lastBuildTime as string), 'PPp') : 'Never'}
                              </p>
                            </div>
                            <div>
                              ...
                              <p className="font-medium flex items-center gap-1">
                                ...
                                {branch.lastBuildDuration ? formatDuration(branch.lastBuildDuration) : '-'}
                              </p>
                            </div>
                          </div>
                          ...
                          {branch.branchUrl && (
                            <div className="text-sm">
                              <p className="text-muted-foreground">Branch URL</p>
                              <p className="text-xs mt-1">
                                <a href={branch.branchUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">
                                  {branch.branchUrl}
                                </a>
                              </p>
                            </div>
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              size="sm"
                              onClick={() => triggerBuild(app.appId, branch.branchName)}
                              disabled={triggeringBuild === `${app.appId}-${branch.branchName}`}
                            >
                              {triggeringBuild === `${app.appId}-${branch.branchName}` ? (
