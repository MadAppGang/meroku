...
export function PostgresConnectionInfo({ config }: PostgresConnectionInfoProps) {
      ...
      <Card>
        ...
        <CardContent>
          <Alert>
            ...
            <AlertDescription>
              PostgreSQL is not enabled. Enable it in the Settings tab to see connection information.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            How your backend service connects to {postgresConfig.aurora ? 'Aurora Serverless v2' : 'RDS PostgreSQL'}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="bg-gray-800 rounded-lg p-4">
            ...
            <div className="space-y-2 text-xs text-gray-300">
              <p>• Backend ECS tasks connect directly to the database endpoint</p>
              <p>• Connection is secured within VPC using security groups</p>
              <p>• {postgresConfig.aurora ? 'Aurora endpoint automatically handles failover' : 'RDS provides a single stable endpoint'}</p>
          ...
          <div className="space-y-3">
            <div className="bg-gray-800 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <code className="text-sm font-mono text-blue-400">PG_DATABASE_HOST</code>
                <Badge variant="outline" className="text-xs">Plaintext</Badge>
              </div>
            ...
            <div className="bg-gray-800 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <code className="text-sm font-mono text-blue-400">PG_DATABASE_USERNAME</code>
                <Badge variant="outline" className="text-xs">Plaintext</Badge>
              </div>
              <p className="text-xs text-gray-400">
            ...
            <div className="bg-gray-800 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <code className="text-sm font-mono text-blue-400">PG_DATABASE_NAME</code>
                <Badge variant="outline" className="text-xs">Plaintext</Badge>
              </div>
              <p className="text-xs text-gray-400">
            ...
            <div className="bg-gray-800 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <code className="text-sm font-mono text-blue-400">PG_DATABASE_PASSWORD</code>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Path: <code>/{config.env}/{config.project}/backend/pg_database_password</code>
              </p>
            </div>
          </div>
          ...
          <div className="bg-yellow-900/10 border border-yellow-600/30 rounded-lg p-3">
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-gray-300">
                The password is injected securely using ECS secrets configuration with <code className="text-yellow-500">valueFrom</code> pointing to the SSM parameter. It never appears in logs or task definitions.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            <div className="bg-gray-800 rounded-lg p-3">
              <h4 className="text-sm font-medium text-gray-200 mb-2">Password Generation</h4>
              <ul className="space-y-1 text-xs text-gray-400">
                <li>• Automatically generated using Terraform's <code>random_password</code> resource</li>
            ...
            <div className="bg-gray-800 rounded-lg p-3">
              <h4 className="text-sm font-medium text-gray-200 mb-2">SSM Parameter Store Locations</h4>
              <div className="space-y-2">
                <div>
                  <code className="text-xs text-blue-400">/{config.env}/{config.project}/postgres_password</code>
                  <p className="text-xs text-gray-500 mt-1">Main password parameter</p>
                </div>
                <div>
                  <code className="text-xs text-blue-400">/{config.env}/{config.project}/backend/pg_database_password</code>
                  <p className="text-xs text-gray-500 mt-1">Backend service copy (used by ECS)</p>
                </div>
              </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="bg-gray-800 rounded-lg p-4">
            <h4 className="text-sm font-medium text-gray-200 mb-3">Connection String Format</h4>
...
{`postgresql://\${PG_DATABASE_USERNAME}:\${PG_DATABASE_PASSWORD}@\${PG_DATABASE_HOST}:5432/\${PG_DATABASE_NAME}`}
          ...
          <div className="bg-gray-800 rounded-lg p-4">
            <h4 className="text-sm font-medium text-gray-200 mb-3">Example in Node.js</h4>
            <div className="bg-gray-900 rounded p-3 overflow-x-auto">
...
const connectionString = \`postgresql://\${process.env.PG_DATABASE_USERNAME}:\${process.env.PG_DATABASE_PASSWORD}@\${process.env.PG_DATABASE_HOST}:5432/\${process.env.PG_DATABASE_NAME}\`;`}
          ...
          <Alert>
            ...
            <AlertDescription className="text-xs">
              <strong>Note:</strong> While the UI mentions <code>DATABASE_URL</code>, the actual implementation 
              provides separate environment variables. Your application needs to construct the connection 
              string from these individual components.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            <div className="bg-gray-800 rounded-lg p-3">
              <h4 className="text-sm font-medium text-gray-200 mb-2">Security Group Configuration</h4>
              <ul className="space-y-1 text-xs text-gray-400">
                ...
                <li>• {postgresConfig.public_access ? 
                  <span className="text-yellow-400">⚠️ Public access is enabled - database is accessible from internet</span> : 
                  <span className="text-green-400">✓ No public access - only VPC internal traffic</span>}
                </li>
              </ul>
            </div>

            <div className="bg-gray-800 rounded-lg p-3">
              <h4 className="text-sm font-medium text-gray-200 mb-2">Additional Security</h4>
              <ul className="space-y-1 text-xs text-gray-400">
                <li>• SSL/TLS encryption for connections</li>
                <li>• {postgresConfig.aurora ? 'Aurora encryption at rest' : 'RDS encryption at rest'}</li>
