...
export function EventTaskProperties({ config, onConfigChange, accountInfo, node }: EventTaskPropertiesProps) {
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <details className="mt-2">
              <summary className="text-xs text-gray-400 cursor-pointer">Common sources</summary>
              <div className="mt-2 space-y-1">
                {commonEventSources.map((example) => (
                  ...
                  >
                    <code className="text-blue-400">{example.value}</code> - {example.description}
                  </div>
                ))}
              </div>
            </details>
          ...
          <div className="space-y-2">
            ...
            <details className="mt-2">
              <summary className="text-xs text-gray-400 cursor-pointer">Common detail types</summary>
              <div className="mt-2 space-y-1">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-gray-500">
              Default: <code className="text-blue-400">{taskEcrRepoUri}:latest</code>
            </p>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xs text-gray-500">Enable for external API calls</p>
            </div>
            <Switch
              id="public-access"
      ...
      <Card>
        ...
        <CardContent>
          <div className="p-3 bg-gray-800 rounded-lg space-y-2">
            ...
            <div className="flex items-center gap-2">
              <code className="text-xs text-blue-400">/{config.env}/{config.project}/task/{taskName}/</code>
              <Button
                size="sm"
                variant="outline"
                onClick={() => {
                  ...
                  window.open(`https://${region}.console.aws.amazon.com/systems-manager/parameters?region=${region}&tab=Table&path=${encodeURIComponent(path)}`, '_blank');
                }}
              >
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-1 gap-3">
              ...
              <div className="flex items-center justify-between p-2 bg-gray-800 rounded">
                ...
                <code className="text-sm text-gray-300">{config.project}_task_{taskName}_{config.env}</code>
              </div>
              <div className="flex items-center justify-between p-2 bg-gray-800 rounded">
              ...
              <div className="flex items-center justify-between p-2 bg-gray-800 rounded">
                ...
                <span className="text-sm text-gray-300">256 units (0.25 vCPU)</span>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="bg-gray-900 rounded-lg p-3 font-mono text-xs text-gray-300 overflow-x-auto">
            <pre>{JSON.stringify({
              source: eventSources.length > 0 ? eventSources : ['<add sources>'],
              'detail-type': detailTypes.length > 0 ? detailTypes : ['<add detail types>']
            }, null, 2)}</pre>
          </div>
          {(eventSources.length === 0 || detailTypes.length === 0) && (
            <Alert className="mt-3">
              ...
              <AlertDescription>
                Add at least one event source and detail type to create a valid event pattern
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
