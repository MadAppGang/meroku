...
export function ScheduledTaskIAMPermissions({
	...
	const scheduledTaskConfig = config.scheduled_tasks?.find(
		(task) => task.name === taskName,
	);
	const eventTaskConfig = config.event_processor_tasks?.find(
		(task) => task.name === taskName,
	);
					...
					? [
							{
								...
								managed: false,
								policyArn:
									(config.sqs as any)?.policy_arn ||
									"Configured via sqs_policy_arn",
							},
						]
					: []),
			],
		...
		<div className="space-y-4">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Three separate roles with specific permissions for scheduled task
						execution
					</CardDescription>
				</CardHeader>
				<CardContent>
					<Alert>
						...
						<AlertDescription>
							Each task uses three IAM roles following the principle of least
							privilege. Roles are automatically created by Terraform with
					...
					<div className="space-y-3">
						<div className="p-3 bg-gray-800 rounded-lg">
							...
							<ul className="text-xs text-gray-400 space-y-1 ml-4">
								...
								<li>
									• Tasks cannot access other services' parameters or resources
								</li>
							</ul>
						</div>
						...
						<div className="p-3 bg-gray-800 rounded-lg">
							<h4 className="text-sm font-medium text-gray-300 mb-2">
								Parameter Store Access Pattern
							</h4>
							...
							<p className="text-xs text-gray-400 mt-2">
								Tasks can only read/write parameters under their designated
								namespace
							</p>
						</div>
						...
						{sqsEnabled && (
							<div className="p-3 bg-blue-900/20 border border-blue-700 rounded-lg">
								...
								<p className="text-xs text-gray-300">
									This task has SQS access enabled. The task role includes the
									SQS policy specified in your configuration.
								</p>
							</div>
						)}
						...
						<div className="p-3 bg-yellow-900/20 border border-yellow-700 rounded-lg">
							...
							<p className="text-xs text-gray-300">
								CloudWatch permissions are quite broad to ensure proper logging
								and monitoring. Consider using more restrictive policies in
