...
export function BackendEnvironmentVariables({
	config,
	accountInfo,
	onConfigChange,
	...
	const automaticEnvVars = [
		{
			...
			value: `${config.project}-${config.env}-rds.${config.region}.rds.amazonaws.com`,
			description: "RDS Instance Endpoint",
		},
		{
			name: "PG_DATABASE_USERNAME",
			value: config.postgres?.username || "postgres",
		...
		{
			name: "AWS_S3_BUCKET",
			value: `${config.project}-backend-${config.env}${config.workload?.bucket_postfix ? `-${config.workload.bucket_postfix}` : ""}`,
		...
		{
			name: "SQS_QUEUE_URL",
			value: `https://sqs.${config.region}.amazonaws.com/${accountInfo?.accountId || config.ecr_account_id || "<ACCOUNT_ID>"}/${config.project}-${config.env}-${config.sqs?.name || "queue"}`,
		...
		<div className="space-y-4">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Variables automatically derived from infrastructure configuration
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="space-y-2">
						{automaticEnvVars.map((envVar) => (
							<div key={envVar.name} className="p-2 bg-gray-800 rounded-lg">
								...
								<div className="text-xs text-gray-300 font-mono break-all mt-1">
									{envVar.value}
								</div>
									...
									<div className="mt-1">
										...
										<div className="flex items-center gap-1 mt-1">
											...
											<Button
												...
												onClick={() => {
													setPortValue(
														config.workload?.backend_image_port?.toString() ||
															"8080",
													);
