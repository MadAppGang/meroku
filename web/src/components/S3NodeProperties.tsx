...
export function S3NodeProperties({
	config,
	onConfigChange,
	...
	const handleAddBucket = () => {
		...
		if (newBucketCors.trim()) {
			try {
				...
				newBucket.cors_rules = Array.isArray(corsRules)
					? corsRules
					: [corsRules];
			} catch (_e) {
	...
	const handleUpdateBucket = (index: number) => {
		...
		if (editBucketCors.trim()) {
			try {
				...
				editBucket.cors_rules = Array.isArray(corsRules)
					? corsRules
					: [corsRules];
			} catch (_e) {
							...
							<div className="space-y-1">
								<div className="flex items-center gap-2">
									...
									<code className="text-sm font-mono text-green-400">
										...
										{config.workload?.bucket_postfix || ""}
									</code>
								</div>
								<div className="flex items-center gap-4 mt-2">
									<div className="flex items-center gap-2">
										...
										<Badge
											variant={
												config.workload?.bucket_public ? "outline" : "secondary"
											}
											className="text-xs"
										>
											{config.workload?.bucket_public
							...
							<div className="space-y-3">
								<div>
									...
									<p className="text-xs text-gray-500 mt-1">
										Final name: {config.project}-{newBucketName || "name"}-
										{config.env}
									</p>
								</div>
									...
									<div className="flex items-center justify-between p-3 bg-gray-900 rounded-lg">
										<div className="space-y-1">
											...
											<p className="text-xs text-gray-400">
												Allow unauthenticated access to objects
											</p>
										</div>
										<Switch
											id="bucket-public"
											...
											onCheckedChange={(checked) => {
												...
												if (checked && !newBucketCors.trim()) {
													setNewBucketCors(
														JSON.stringify(defaultPublicCors, null, 2),
													);
												}
											}}
										/>
								...
								<div>
									...
									<Textarea
										...
										placeholder={
											newBucketPublic
												? "Default CORS for public access will be applied"
												: `{
									...
									<div className="space-y-4">
										<div>
											...
											<Input
												...
												onChange={(e) =>
													setEditBucket({ ...editBucket, name: e.target.value })
												}
												className="font-mono text-sm"
											/>
											<p className="text-xs text-gray-500 mt-1">
												Full name: {config.project}-{editBucket.name || "name"}-
												{config.env}
											</p>
										</div>
										...
										<div>
											...
											<Textarea
												...
												placeholder={
													editBucket.public
														? "Default CORS for public access will be applied if empty"
														: "Optional CORS rules in JSON format"
												}
										...
										<div className="space-y-2">
											...
											<div className="flex items-center gap-3 text-xs">
												...
												<div className="flex items-center gap-1">
													...
													<span className="text-gray-400">
														Versioning{" "}
														{bucket.versioning === false ? "Off" : "On"}
													</span>
												</div>
												{bucket.cors_rules && (
													<div className="flex items-center gap-1">
					...
					{buckets.length === 0 && !showNewBucket && (
						<div className="text-center py-8 text-gray-400">
							...
							<p className="text-xs mt-1">
								Click "Add Bucket" to create a new S3 bucket
							</p>
						</div>
					)}
				</CardContent>
