...
export function ScheduledTaskCloudWatch({
	...
	const logGroup = {
		...
		service:
			node.type === "scheduled-task" ? "ECS Scheduled Task" : "ECS Event Task",
		retention: "7 days",
	};
			...
			<Card>
				...
				<CardContent>
					...
					<Alert className="mt-4">
						...
						<AlertDescription>
							Each task execution creates a new log stream. Logs are retained
							for 7 days. Task runs are triggered based on the schedule
							expression.
						</AlertDescription>
					</Alert>
				</CardContent>
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between">
							...
							<Button
								...
								onClick={() =>
									copyCommand(
										generateLastExecutionCommand(logGroup.name),
										"last-exec",
									)
								}
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between">
							...
							<Button
								...
								variant="outline"
								onClick={() =>
									copyCommand(generateTailCommand(logGroup.name), "tail")
								}
								className="h-6 px-2 text-xs"
							>
								<Copy className="w-3 h-3 mr-1" />
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between">
							...
							<Button
								...
								variant="outline"
								onClick={() =>
									copyCommand(generateFilterCommand(logGroup.name), "filter")
								}
								className="h-6 px-2 text-xs"
							>
								<Copy className="w-3 h-3 mr-1" />
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between">
							...
							<Button
								...
								onClick={() =>
									copyCommand(
										generateFilterCommandWithQuery(logGroup.name),
										"error",
									)
								}
								className="h-6 px-2 text-xs"
				...
				<CardContent className="space-y-4">
					<div className="bg-blue-900/20 border border-blue-700 rounded-lg p-4">
						<p className="text-xs text-gray-300 mb-3">
							Use CloudWatch Insights to analyze task execution patterns,
...
| limit 50`}</pre>
					...
					<Alert>
						...
						<AlertDescription>
							...
							Select log group <code className="text-xs">{logGroup.name}</code>{" "}
							and paste the query.
						</AlertDescription>
					</Alert>
				</CardContent>
