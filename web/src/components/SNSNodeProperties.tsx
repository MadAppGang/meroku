...
import { Bell, Info, Key, Loader2, Check, AlertCircle, Eye, EyeOff, RefreshCw } from 'lucide-react';
...
export function SNSNodeProperties({ config, onConfigChange }: SNSNodePropertiesProps) {
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Enable Firebase Cloud Messaging platform application for push notifications
            </p>
          </div>
          <Switch
            id="sns-enabled"
        ...
        {isEnabled && (
          <>
            <div className="mt-4 p-3 bg-gray-800 rounded-lg space-y-3">
              <h4 className="text-sm font-medium text-gray-200">Configuration Details</h4>
              
              <div className="space-y-2 text-sm">
                <div className="flex items-start gap-2">
                  <span className="text-gray-400 min-w-[140px]">Platform App Name:</span>
                  <code className="text-pink-400 font-mono">{config.project}-fcm-{config.env}</code>
                </div>
                
                <div className="flex items-start gap-2">
                  <span className="text-gray-400 min-w-[140px]">Platform:</span>
                  <span className="text-gray-200">GCM (Google Cloud Messaging)</span>
                </div>
                
                <div className="flex items-start gap-2">
                  <span className="text-gray-400 min-w-[140px]">Server Key Path:</span>
                  <code className="text-blue-400 font-mono text-xs break-all">/{config.env}/{config.project}/backend/gcm-server-key</code>
                </div>
              </div>
            </div>
            ...
            <div className="p-3 bg-gray-800 rounded-lg space-y-3">
              <div className="flex items-center justify-between">
                <h4 className="text-sm font-medium text-gray-200">GCM/FCM Server Key</h4>
                {hasLoaded && (
                  <Button
              ...
              {!hasLoaded ? (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <p className="text-gray-500">
                      ...
                      Will be stored at: <code className="text-blue-400">{parameterPath}</code>
                    </p>
                    <Button
                      onClick={loadParameter}
                      disabled={loading}
                ...
                <>
                  ...
                  <p className="text-xs text-gray-500">
                    <Key className="w-3 h-3 inline mr-1" />
                    Stored at: <code className="text-blue-400">{parameterPath}</code>
                  </p>
            ...
            <div className="p-3 bg-gray-800 rounded-lg">
              <h4 className="text-sm font-medium text-gray-200 mb-2">IAM Permissions</h4>
              <p className="text-xs text-gray-400 mb-2">The backend service will have these SNS permissions:</p>
        ...
        <Alert className="border-blue-600">
          ...
          <AlertDescription className="text-xs">
            <strong>Note:</strong> This configuration is specifically for Firebase Cloud Messaging (FCM) push notifications. 
            For other SNS features like topics, email subscriptions, or SMS, additional Terraform configuration would be required.
          </AlertDescription>
        </Alert>
      </CardContent>
    </Card>
