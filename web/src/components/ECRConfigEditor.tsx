...
export const ECRConfigEditor = memo(function ECRConfigEditor({
	...
	useEffect(() => {
		...
		if (prev.onEcrConfigChange !== onEcrConfigChange)
			changes.push("onEcrConfigChange");
		...
		if (changes.length > 0) {
			console.log(`🔧 [ECRConfigEditor] Props changed: ${changes.join(", ")}`, {
				...
				onChangeRef:
					prev.onEcrConfigChange === onEcrConfigChange ? "same" : "CHANGED",
			});
		}
	...
	console.log(
		`🔄 [ECRConfigEditor] Render #${renderCountRef.current} for ${currentServiceType}/${currentServiceName}`,
	);

	if (renderCountRef.current > 50) {
		console.error(
			"⚠️ [ECRConfigEditor] INFINITE LOOP DETECTED - More than 50 renders!",
		);
	...
	const availableSources = useDeepMemo(() => {
		console.log(
			`🔍 [ECRConfigEditor] Recalculating availableSources (deep compare) for ${currentServiceName}`,
		);
		const sources: Array<{
			name: string;
		...
		config.services?.forEach((svc) => {
			const isCurrentService =
				currentServiceType === "services" && svc.name === currentServiceName;
			if (
				!isCurrentService &&
		...
		config.scheduled_tasks?.forEach((st) => {
			const isCurrentService =
				...
				st.name === currentServiceName;
			if (
				!isCurrentService &&
				(!st.ecr_config || st.ecr_config.mode === "create_ecr")
			) {
				sources.push({
					name: st.name,
								...
								<div>
									<p className="text-xs text-gray-300">
										...
										<span className="font-mono text-gray-400">
											{ecrConfig.source_service_type?.replace("_", " ")} /{" "}
