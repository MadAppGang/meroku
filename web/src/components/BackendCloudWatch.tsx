import { Clock, Cloud, Copy, FileText, Info, Terminal } from "lucide-react";
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						...
						{isService
							? `${serviceName} service`
							: "backend services and collectors"}
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="space-y-4">
					...
					<Alert className="mt-4">
						...
						<AlertDescription>
							Logs are automatically created when services start. Retention is
							set to 7 days by default. Log streams are created per task
							instance.
						</AlertDescription>
					</Alert>
				</CardContent>
							...
							<div className="space-y-2">
								<div className="flex items-center justify-between">
									...
									<Button
										...
										onClick={() =>
											copyCommand(
												generateTailCommand(group.name),
												`tail-${groupIndex}`,
											)
										}
							...
							<div className="space-y-2">
								<div className="flex items-center justify-between">
									...
									<Button
										...
										onClick={() =>
											copyCommand(
												generateFilterCommand(group.name),
												`filter-${groupIndex}`,
											)
										}
										className="h-6 px-2 text-xs"
									>
										<Copy className="w-3 h-3 mr-1" />
										{copiedCommand === `filter-${groupIndex}`
											? "Copied!"
											: "Copy"}
									</Button>
								</div>
							...
							<div className="space-y-2">
								<div className="flex items-center justify-between">
									...
									<Button
										...
										onClick={() =>
											copyCommand(
												generateFilterCommandWithQuery(group.name),
												`error-${groupIndex}`,
											)
										}
										className="h-6 px-2 text-xs"
									>
										<Copy className="w-3 h-3 mr-1" />
										{copiedCommand === `error-${groupIndex}`
											? "Copied!"
											: "Copy"}
									</Button>
								</div>
				...
				<CardContent className="space-y-4">
					<div className="bg-blue-900/20 border border-blue-700 rounded-lg p-4">
						<p className="text-xs text-gray-300 mb-3">
							CloudWatch Insights provides a powerful query language for
							analyzing log data. Here are some useful queries:
						</p>
					</div>
...
| limit 10`}</pre>
					...
					<Alert>
						...
						<AlertDescription>
							You can run these queries in the AWS Console under CloudWatch →
							Logs → Insights. Select your log group and paste the query.
						</AlertDescription>
					</Alert>
				</CardContent>
			</Card>
					...
					<div className="space-y-3">
						<div className="p-3 bg-gray-800 rounded-lg space-y-2">
							...
							<ul className="text-xs text-gray-400 space-y-1 ml-4">
								<li>
									• <code className="text-blue-400">--format short</code> -
									Simplified output format
								</li>
								<li>
									• <code className="text-blue-400">--since 1h</code> - Show
									logs from last hour
								</li>
								<li>
									•{" "}
								...
								<li>
									•{" "}
									<code className="text-blue-400">--profile your-profile</code>{" "}
									- Use specific AWS profile
								</li>
								<li>
									• <code className="text-blue-400">--region us-east-1</code> -
									Specify AWS region
								</li>
							</ul>
						</div>
