...
const buildStatusConfig: Record<string, { color: string; bgColor: string; icon: any; text: string; priority: number; pulse?: boolean }> = {
  SUCCEED: { 
...
export function AmplifyStatusWidget({
  ...
  }, [appName, environment, profile, autoRefresh, refreshInterval]);
  ...
  const getOverallStatus = () => {
    ...
    const statuses = app.branches.map(b => ({
      ...
      config: buildStatusConfig[b.lastBuildStatus as keyof typeof buildStatusConfig] || buildStatusConfig.PENDING,
      branchName: b.branchName,
    }));
    
    statuses.sort((a, b) => (b.config.priority || 0) - (a.config.priority || 0));
    return statuses[0];
  };
  ...
  if (loading && !app) {
    return (
      <div className={cn("flex items-center gap-2", className)}>
        ...
        {variant !== 'minimal' && <span className="text-sm text-muted-foreground">Checking status...</span>}
      </div>
    );
  }
    ...
    return (
      <div className={cn("flex items-center gap-2", className)}>
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-gray-500/10">
          ...
          <span className="text-sm font-medium text-gray-500">Status Unavailable</span>
        </div>
      </div>
    );
  }
  ...
  const branchStatuses = app.branches.map(branch => {
    const config = buildStatusConfig[branch.lastBuildStatus as keyof typeof buildStatusConfig] || buildStatusConfig.PENDING;
      ...
      <div className="space-y-2">
        <div className={cn("flex items-center gap-2", className)}>
          <div className={cn(
            ...
            <span className={cn("text-sm font-medium", overallStatus.config.color)}>
