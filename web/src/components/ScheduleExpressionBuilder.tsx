...
function MultiSelectField({ value, onChange, options, placeholder }: MultiSelectProps) {
...
export function ScheduleExpressionBuilder({ value, onChange }: ScheduleExpressionBuilderProps) {
  // Parse the initial value to determine type and values
  const isRate = value.startsWith("rate(");
  const isCron = value.startsWith("cron(");
  
  const [expressionType, setExpressionType] = useState<"rate" | "cron">(isRate ? "rate" : "cron");
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Select value={preset} onValueChange={applyPreset}>
          ...
          <SelectContent>
            ...
            <SelectItem value="first-monday">First Monday of month at 9 AM UTC</SelectItem>
          </SelectContent>
        </Select>
      </div>
      
      {/* Expression Type Tabs */}
      <Tabs value={expressionType} onValueChange={(v) => setExpressionType(v as "rate" | "cron")}>
        ...
        <TabsContent value="rate" className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label>Unit</Label>
              <Select value={rateUnit} onValueChange={(v) => setRateUnit(v as any)}>
                <SelectTrigger className="bg-gray-800 border-gray-600">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="text-xs text-gray-400">
            Runs every {rateValue} {rateUnit}{parseInt(rateValue) !== 1 ? 's' : ''}
          </div>
        </TabsContent>
        ...
        <TabsContent value="cron" className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <ul className="text-xs text-gray-500 space-y-1">
              ...
              <li>• <code>?</code> - No specific value (day of month/week)</li>
              <li>• <code>-</code> - Range (e.g., MON-FRI)</li>
              <li>• <code>,</code> - List (e.g., MON,WED,FRI)</li>
              <li>• <code>#</code> - Nth occurrence (e.g., 2#1 = first Monday)</li>
            </ul>
          </div>
        </TabsContent>
      </Tabs>
      ...
      <div className="p-3 bg-gray-800 rounded-lg">
        ...
        <code className="text-sm text-blue-300 font-mono">
          {expressionType === "rate" ? buildRateExpression() : buildCronExpression()}
        </code>
      </div>
    </div>
  );
