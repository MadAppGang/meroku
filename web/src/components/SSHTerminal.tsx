...
export function SSHTerminal({ env, serviceName, taskArn, containerName, onClose }: SSHTerminalProps) {
  ...
  useEffect(() => {
    ...
    const ws = wsManager.current.getConnection(connectionKey.current, {
      ...
      onOpen: () => {
        ...
        if (wsManager.current.isConnected(connectionKey.current)) {
          console.log('WebSocket is confirmed open, waiting for backend connected message');
        }
      },
      ...
      onError: (error) => {
        updateConnectionStatus('error');
        const errorMsg = error.message || 'Failed to establish WebSocket connection';
        setError(errorMsg);
        const currentTerminal = terminalInstance.current;
        if (currentTerminal) {
          currentTerminal.writeln(`\n\x1b[1;31mConnection error: ${errorMsg}\x1b[0m`);
          currentTerminal.writeln(`\x1b[1;33mPlease ensure the backend server is running and accessible.\x1b[0m`);
        }
      },
      onClose: () => {
        updateConnectionStatus('disconnected');
    ...
    const connectionTimeout = setTimeout(() => {
      if (!wsManager.current.isConnected(connectionKey.current)) {
        ...
        if (currentTerminal) {
          ...
          currentTerminal.writeln('\x1b[1;33mThe backend server may not have the SSH endpoint implemented.\x1b[0m');
        }
      }
    }, 10000);
    ...
    terminal.onData((data) => {
      console.log('Terminal input:', JSON.stringify(data), 'Connection status:', connectionStatusRef.current);
      // Use the manager to check connection and send data
      if (wsManager.current.isConnected(connectionKey.current) && connectionStatusRef.current === 'connected') {
  ...
  return (
    <div className={`fixed ${isFullScreen ? 'inset-0' : 'inset-4'} z-50 bg-gray-950 flex flex-col transition-all duration-200`}>
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-gray-700 bg-gray-900">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-400">Task: {taskId.substring(0, 12)}...</span>
      ...
      {/* Terminal */}
      <div className="flex-1 overflow-hidden bg-black p-4" style={{ minHeight: '300px' }}>
        <div 
          ref={terminalRef} 
