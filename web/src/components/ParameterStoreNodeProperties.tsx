import { Clock, Info, Key, Package, Server } from "lucide-react";
...
export function ParameterStoreNodeProperties({
	config,
}: ParameterStoreNodePropertiesProps) {
						...
						<div className="flex items-start gap-2">
							...
							<div className="flex-1">
								...
								<ul className="text-xs text-gray-300 space-y-1">
									<li>
										• <strong>Hierarchical Organization:</strong> Parameters
										organized by /{config.env}/{config.project}/{"{component}"}/
									</li>
									<li>
										• <strong>SecureString Encryption:</strong> All sensitive
										data encrypted using KMS
									</li>
									<li>
										• <strong>Lifecycle Management:</strong> Parameters have{" "}
										<code className="text-blue-300">
											ignore_changes = [value]
										</code>{" "}
										to prevent Terraform overwriting manual updates
									</li>
									<li>
										• <strong>Auto-Discovery:</strong> Services use{" "}
										<code className="text-blue-300">
											aws_ssm_parameters_by_path
										</code>{" "}
										to automatically load all parameters in their namespace
									</li>
									<li>
										• <strong>Tagging:</strong> All parameters tagged with
										Environment, Project, ManagedBy, and Application
									</li>
								</ul>
							</div>
						</div>
										...
										<div className="flex items-start gap-3">
											...
											<div className="flex-1 space-y-1">
												...
												<div className="flex items-center gap-2">
													<span
														className={`text-xs ${param.type === "SecureString" ? "text-green-400" : "text-blue-400"}`}
													>
														Type: {param.type}
													</span>
													{param.note && (
...
# - /${config.env}/${config.project}/backend/gcm-server-key (if enabled)`}
						</pre>
					</div>
					...
					<div className="bg-green-900/20 border border-green-700 rounded-lg p-4">
						...
						<ul className="text-xs text-gray-300 space-y-1">
							<li>
								• <strong>Security:</strong> Sensitive data like passwords and
								API keys are encrypted
							</li>
							<li>
								• <strong>Flexibility:</strong> Operators can manually
								add/update parameters without modifying Terraform
							</li>
							<li>
								• <strong>Separation:</strong> Each service has its own
								parameter namespace
							</li>
							<li>
								• <strong>Integration:</strong> ECS tasks automatically inject
