...
export default function ServiceCICDConfiguration({
	...
	const isTask =
		serviceType === "scheduled-task" || serviceType === "event-task";
				...
				<div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
					...
					<div className="mt-3 space-y-2">
						<p className="text-xs text-slate-400">
							This event will be matched by EventBridge rules configured to
							listen for{" "}
						...
						<p className="text-xs text-slate-400">
							<strong className="text-slate-300">Deployment Flow:</strong>{" "}
							EventBridge → Lambda Function → ECS Service Update
						</p>
						<p className="text-xs text-slate-400">
							The deployment Lambda function will receive this event, pull the
							latest image from the cross-account ECR, and trigger an ECS
							service update with the new image. ECS redeployment typically
							takes <strong className="text-slate-300">1-3 minutes</strong>.
						</p>
						<p className="text-xs text-slate-400">
							💡 <strong className="text-slate-300">Tip:</strong> You can
							configure the Lambda function with a Slack webhook URL to receive
							deployment notifications in your Slack channel (start, success, or
							failure status).
						</p>
					</div>
				</div>
				...
				<div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
					...
					<div className="space-y-3">
						...
						<div className="space-y-2">
							<p className="text-xs text-slate-400">
								<strong className="text-slate-300">Deployment Flow:</strong>{" "}
								GitHub Actions → AWS ECS Service Update
							</p>
							<p className="text-xs text-slate-400">
								This workflow deploys the pre-built Docker image from Docker
								Hub, GHCR, or any other container registry. The workflow does
								not build a new image; it triggers a deployment with the
								existing image configured in the service. ECS redeployment
								typically takes{" "}
								<strong className="text-slate-300">1-3 minutes</strong>.
							</p>
							<p className="text-xs text-slate-400">
								💡 <strong className="text-slate-300">Tip:</strong> Make sure
								the ECS task definition references the correct image URI. Update
								the image tag in your service configuration when you want to
