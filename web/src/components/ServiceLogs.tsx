...
import { infrastructureApi, type LogEntry } from "../api/infrastructure";
...
export function ServiceLogs({ environment, serviceName }: ServiceLogsProps) {
  ...
  const idBase = useId();
  const autoScrollId = `${idBase}-auto-scroll`;
  const autoScrollFullscreenId = `${idBase}-auto-scroll-fullscreen`;
  ...
  const exportLogs = () => {
    const content = filteredLogs
      .map(
        (log) =>
          `[${log.timestamp}] [${log.level.toUpperCase()}] [${log.stream}] ${
            log.message
          }`
      )
      .join("\n");
      ...
      <div className="space-y-4">
            ...
            <Button
              ...
              title={
                isStreaming
                  ...
                  : "Stream logs from last 24 hours + new logs"
              }
            >
              {isStreaming ? (
                <>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <Label htmlFor={autoScrollId} className="text-sm">
              Auto-scroll
            </Label>
          </div>
        </div>
                ...
                {!isStreaming &&
                  ...
                  filterLevel === "all" && (
                    <div className="mt-4 space-y-2">
                      ...
                      <p className="text-xs">
                        • Click "Start Streaming" to see logs from the last 24
                        hours
                      </p>
                      <p className="text-xs">
                        • Check if the service is running and generating logs
                      </p>
                    </div>
                  )}
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-gray-950 flex flex-col">
          ...
          <div className="p-4 border-b border-gray-700 bg-gray-900">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <Switch
                  id={autoScrollFullscreenId}
                  checked={autoScroll}
                  onCheckedChange={setAutoScroll}
                />
                <Label htmlFor={autoScrollFullscreenId} className="text-sm">
                  Auto-scroll
                </Label>
              </div>
                  ...
                  filteredLogs.map((log, index) => (
                    ...
                    >
                      <div className="flex items-start gap-3 mb-2">
                        ...
                        <span
                          className="text-gray-400 text-sm truncate max-w-[300px]"
                          title={log.stream}
                        >
                          [{log.stream}]
                        </span>
