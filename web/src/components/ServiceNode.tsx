		...
		>
          ...
          ${
			...
			<div className="flex items-center gap-3 mb-2">
				<div
					className={`p-2 rounded-lg ${serviceColor} ${data.disabled ? "opacity-60" : ""}`}
				>
			...
			{isServiceNode && data.configProperties && (
				<div className="space-y-2 mt-2">
					...
					{(data.type === "backend" || data.type === "service") &&
						data.configProperties.domain && (
						...
						<div className="text-xs bg-gray-900/50 rounded px-2 py-1">
							<div className="flex items-center gap-1 text-gray-400">
								...
								<span className="font-mono">
									{data.configProperties.schedule}
								</span>
							</div>
						</div>
					)}
					...
					{data.type === "amplify" && data.configProperties && (
						<>
							...
							{/* Branch Info */}
							{data.configProperties.branches &&
								data.configProperties.branches.length > 0 && (
									<div className="text-xs bg-gray-900/50 rounded px-2 py-1 mb-2">
										<div className="flex items-center gap-1 text-gray-400">
											<GitBranch className="w-3 h-3" />
											<span>
												{data.configProperties.branches.length} branch
												{data.configProperties.branches.length !== 1
													? "es"
													: ""}
											</span>
									...
									data.configProperties.autoscalingEnabled ? (
										<div className="flex items-center gap-0.5">
											...
											<span className="text-white font-medium">
												{data.configProperties.autoscalingMinCapacity || 1}-
												{data.configProperties.autoscalingMaxCapacity || 10}{" "}
												instances
											</span>
										</div>
									) : (
										data.configProperties.desiredCount && (
											<div className="flex items-center gap-0.5">
												...
												<span className="text-white font-medium">
													{data.configProperties.desiredCount} instance
													{data.configProperties.desiredCount !== 1 ? "s" : ""}
												</span>
											</div>
										)
									)}
			...
			{data.deploymentType && !isServiceNode && (
				<div className="text-xs text-gray-400 mb-2">{data.deploymentType}</div>
			)}
