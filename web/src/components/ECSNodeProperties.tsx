...
import {
	...
	infrastructureApi,
	type ECSNetworkInfo as NetworkInfo,
...
export function ECSNodeProperties({
	config,
	onConfigChange,
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 gap-4">
						...
						<div>
							...
							/>
							<p className="text-xs text-gray-500 mt-1">
								AWS region for all resources (e.g., us-east-1, eu-central-1)
							</p>
						</div>
							...
							<div className="flex items-start gap-2">
								...
								<div className="flex-1">
									...
									<p className="text-xs text-gray-300">
										This environment is marked as production. This affects
										domain prefixing and other security settings.
									</p>
								</div>
							</div>
						</div>
						...
						<div className="flex items-start gap-2">
							...
							<div className="flex-1">
								...
								<ul className="text-xs text-gray-400 space-y-1">
									<li>
										• <strong>Project/Region:</strong> Requires destroying and
										recreating all resources
									</li>
									<li>
										• <strong>State Bucket/File:</strong> Requires Terraform
										state migration
									</li>
									<li>
										• <strong>Production Flag:</strong> May affect domain
										configuration and security policies
									</li>
								</ul>
								<p className="text-xs text-gray-300 mt-2">
									Always run{" "}
									<code className="text-blue-300">make infra-plan</code> before
									applying changes.
								</p>
							</div>
						</div>
				...
				<CardContent className="space-y-4">
					...
					<div className="bg-blue-900/20 border border-blue-700 rounded-lg p-4">
						<div className="flex items-start gap-2">
							...
							<div className="flex-1">
								...
								<p className="text-xs text-gray-300">
									This ECS cluster runs all your containerized services using
									AWS Fargate. It provides serverless compute for containers
									with automatic scaling and high availability.
								</p>
							</div>
						</div>
					</div>
					...
					<div className="bg-gray-800 rounded-lg p-4">
						...
						<p className="text-xs text-gray-400">
							Services use AWS Cloud Map for service discovery with the
							namespace <code className="text-blue-300">local</code>. This
							allows services to communicate using DNS names like{" "}
							<code className="text-blue-300">backend.local</code>.
						</p>
					</div>
					...
					<div className="bg-yellow-900/20 border border-yellow-700 rounded-lg p-4">
						...
						<p className="text-xs text-gray-300">
							...
							CPU and memory utilization. Services automatically scale in and
							out based on demand.
						</p>
					</div>
				</CardContent>
...
export function ECSClusterInfo({
	config,
}: {
	config: YamlInfrastructureConfig;
}) {
	...
	useEffect(() => {
		const fetchClusterInfo = async () => {
			try {
				...
				setError(
					err instanceof Error ? err.message : "Failed to fetch cluster info",
				);
			} finally {
				setLoading(false);
			}
					...
					<div className="grid grid-cols-1 gap-4">
						...
						<div>
							...
							<p className="text-sm text-gray-300">
								...
								>
									{clusterInfo.containerInsights === "enabled" ? (
										<CheckCircle className="w-3 h-3" />
									) : (
...
export function ECSNetworkInfo({
	config,
}: {
	config: YamlInfrastructureConfig;
}) {
	...
	const privateSubnets = networkInfo.subnets.filter(
		(s) => s.type === "private",
	);
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 gap-4">
						<div>
							...
							<p className="text-xs text-gray-500 mt-1">
								CIDR: {networkInfo.vpc.cidrBlock}
							</p>
							<p className="text-xs text-gray-500">
								State:{" "}
								<span className="text-green-400">{networkInfo.vpc.state}</span>
							</p>
						</div>
										...
										<ul className="text-xs text-gray-500 space-y-1">
											{privateSubnets.map((subnet) => (
												<li key={subnet.subnetId}>
													...
													{subnet.availableIpCount} IPs available
												</li>
											))}
										</ul>
										<p className="text-xs text-gray-400 mt-2">
											Used for ECS tasks and RDS instances
										</p>
									</div>
								)}
						...
						{networkInfo.serviceDiscovery && (
							<div>
								...
								<p className="text-xs text-gray-500 mt-1">
									{networkInfo.serviceDiscovery.serviceCount} service
									{networkInfo.serviceDiscovery.serviceCount !== 1 ? "s" : ""}{" "}
									registered
								</p>
								<p className="text-xs text-gray-500">
									Services can discover each other using DNS names like{" "}
									<code>
										service-name.{networkInfo.serviceDiscovery.namespaceName}
									</code>
								</p>
							</div>
						)}
							...
							{networkInfo.subnets.map((subnet) => (
								<div key={subnet.subnetId} className="text-xs">
									...
									<p className="text-gray-500 ml-2">
										{subnet.type === "private" ? "🔒" : "🌐"} {subnet.cidrBlock}{" "}
										• {subnet.availabilityZone} • {subnet.availableIpCount} IPs
									</p>
								</div>
							))}
						</div>
...
export function ECSServicesInfo({
	config,
}: {
	config: YamlInfrastructureConfig;
}) {
	...
	useEffect(() => {
		const fetchServicesInfo = async () => {
			try {
				...
				setError(
					err instanceof Error ? err.message : "Failed to fetch services info",
				);
			} finally {
				setLoading(false);
			}
					...
					<div className="flex items-center justify-between">
						...
						<Button
							...
							onClick={() => {
								...
								setError(null);
								infrastructureApi
									.getECSServicesInfo(config.env)
									.then(setServicesInfo)
									.catch((err) =>
										setError(
											err instanceof Error
												? err.message
												: "Failed to fetch services info",
										),
									)
									.finally(() => setLoading(false));
							}}
				...
				<CardContent className="space-y-4">
					...
					<div className="grid grid-cols-2 gap-4">
						...
						<div className="bg-gray-800 rounded-lg p-4">
							...
							<p className="text-2xl font-medium text-gray-300 mb-3">
								{servicesInfo.totalTasks}
							</p>
							<div className="space-y-1">
								<p className="text-xs text-gray-500">
									• Service Tasks: {runningServicesCount}
								</p>
								...
								{servicesInfo.scheduledTasks.map((task) => (
									<div key={task.taskName} className="p-3 bg-gray-800 rounded">
										<div className="flex items-center justify-between gap-2">
											...
											<span
												className={`text-xs flex-shrink-0 ${task.enabled ? "text-green-400" : "text-gray-400"}`}
											>
					...
					<div className="bg-green-900/20 border border-green-700 rounded-lg p-4">
						...
						<p className="text-xs text-gray-300">
							Each service is allocated CPU and memory based on workload
							requirements. Fargate automatically provisions the right compute
							resources.
						</p>
					</div>
				</CardContent>
...
export function ECSNotifications({
	config,
	onConfigChange,
}: ECSNodePropertiesProps) {
	...
	const testSlackWebhook = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				setWebhookTestResult({
					success: false,
					message: `Failed to send test notification: ${response.status}`,
				});
			}
		} catch (error) {
			setWebhookTestResult({
				...
				<CardContent className="space-y-4">
					<div>
						...
						<div className="mt-1 space-y-2">
							...
							{webhookTestResult && (
								<div
									className={`text-xs p-2 rounded ${webhookTestResult.success ? "bg-green-900/20 text-green-400" : "bg-red-900/20 text-red-400"}`}
								>
						...
						<div className="bg-green-900/20 border border-green-700 rounded-lg p-4">
							<div className="flex items-start gap-2">
								...
								<div className="flex-1">
									...
									<p className="text-xs text-gray-300">
										Deployment events and critical alerts will be sent to your
										Slack channel.
									</p>
					...
					{!config.workload?.slack_webhook && (
						<div className="bg-gray-800 rounded-lg p-4">
							<p className="text-xs text-gray-400">
								<strong>Note:</strong> Without a Slack webhook, deployment
								notifications will only be available in CloudWatch Logs.
							</p>
						</div>
					)}
				</CardContent>
				...
				<CardContent>
					<ol className="space-y-3 text-xs text-gray-300">
						<li className="flex items-start gap-2">
							...
							<span>
								Go to your Slack workspace and navigate to <strong>Apps</strong>
							</span>
						</li>
						<li className="flex items-start gap-2">
							<span className="text-blue-400 font-medium flex-shrink-0">
							...
							<span>
								Search for and add <strong>"Incoming Webhooks"</strong>
							</span>
						</li>
						<li className="flex items-start gap-2">
							<span className="text-blue-400 font-medium flex-shrink-0">
					...
					<div className="mt-4 p-3 bg-gray-800 rounded">
						<p className="text-xs text-gray-400">
							<strong>Tip:</strong> Create a dedicated channel like{" "}
							<code>#deployments</code> or{" "}
							<code>
								#{config.project}-{config.env}
							</code>{" "}
							for infrastructure notifications.
						</p>
					</div>
				</CardContent>
			</Card>
				...
				<CardContent>
					<div className="space-y-3">
						<div className="p-3 bg-gray-800 rounded-lg">
							...
							<p className="text-xs text-gray-400">
								All deployment events are automatically logged to CloudWatch.
								Access them through the AWS Console or CLI.
							</p>
						</div>
						...
						<div className="p-3 bg-gray-800 rounded-lg">
							...
							<p className="text-xs text-gray-400">
								ECS deployment events are published to EventBridge, allowing
								custom integrations with SNS, Lambda, or other services.
							</p>
						</div>
						...
						<div className="p-3 bg-gray-800 rounded-lg">
							...
							<p className="text-xs text-gray-400">
								Future support planned for: Email notifications, PagerDuty,
								Discord webhooks, and custom webhook endpoints.
							</p>
						</div>
					</div>
				</CardContent>
