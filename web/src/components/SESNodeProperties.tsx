        ...
        <CardContent>
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <Label htmlFor="ses-enabled" className="text-base">Enable SES</Label>
        ...
        <>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure the domain for sending emails (uses mail subdomain by default)
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {actualDomain && (
                <Alert>
                  ...
                  <AlertDescription>
                    <div className="space-y-1">
                      <p>SES will use: <strong className="text-blue-400">{actualDomain}</strong></p>
                      <p className="text-xs">DNS records will be automatically created in Route53 for domain verification and DKIM authentication</p>
                    </div>
                  </AlertDescription>
                </Alert>
              )}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Email addresses to verify for testing (useful in SES sandbox mode)
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Alert>
                ...
                <AlertDescription>
                  Test emails are useful when SES is in sandbox mode. Each email address will be verified individually.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div className="flex items-start gap-3 p-3 bg-gray-800 rounded-lg">
                    ...
                    <div className="flex-1">
                      <h4 className="text-sm font-medium text-gray-200">Domain Identity</h4>
                      <p className="text-xs text-gray-400 mt-1">
                        Verifies ownership of {actualDomain || 'your domain'} for sending emails
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 p-3 bg-gray-800 rounded-lg">
                    <CheckCircle className="w-5 h-5 text-green-400 mt-0.5" />
                    <div className="flex-1">
                      <h4 className="text-sm font-medium text-gray-200">DKIM Configuration</h4>
                      <p className="text-xs text-gray-400 mt-1">
                        Sets up DomainKeys Identified Mail for email authentication
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 p-3 bg-gray-800 rounded-lg">
                    <CheckCircle className="w-5 h-5 text-green-400 mt-0.5" />
                    <div className="flex-1">
                      <h4 className="text-sm font-medium text-gray-200">DNS Records in Route53</h4>
                  ...
                  <div className="flex items-start gap-3 p-3 bg-gray-800 rounded-lg">
                    <CheckCircle className="w-5 h-5 text-green-400 mt-0.5" />
                    <div className="flex-1">
                      <h4 className="text-sm font-medium text-gray-200">Email Identities</h4>
                      <p className="text-xs text-gray-400 mt-1">
                        Verifies {testEmails.length} test email {testEmails.length === 1 ? 'address' : 'addresses'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 p-3 bg-gray-800 rounded-lg">
                    <CheckCircle className="w-5 h-5 text-green-400 mt-0.5" />
                    <div className="flex-1">
                      <h4 className="text-sm font-medium text-gray-200">ACM Certificate</h4>
                      <p className="text-xs text-gray-400 mt-1">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Key information about using SES
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <Alert>
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  <strong>Sandbox Mode:</strong> SES starts in sandbox mode with sending restrictions. 
                  You need to request production access from AWS to send to unverified addresses.
                </AlertDescription>
              </Alert>
          ...
          <Card>
            ...
            <CardContent>
              <div className="bg-gray-900 rounded-lg p-3 font-mono text-xs text-gray-300 overflow-x-auto">
                <pre>{`ses:
  enabled: ${sesConfig.enabled}${sesConfig.domain_name ? `
  domain_name: "${sesConfig.domain_name}"` : isDomainEnabled && mainDomain ? `
  # domain_name not specified - will use default: ${defaultDomain}` : `
  domain_name: "example.com" # Required - domain module not enabled`}${testEmails.length > 0 ? `
  test_emails:${testEmails.map(email => `
    - "${email}"`).join('')}` : ''}`}</pre>
              </div>
            </CardContent>
          </Card>
