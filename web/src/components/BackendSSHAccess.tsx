...
export function BackendSSHAccess({
	config,
	onConfigChange,
	accountInfo,
	isService = false,
	...
	const [activeSSHSession, setActiveSSHSession] = useState<{
		taskArn: string;
	...
	const fetchTasks = async () => {
		try {
			...
			const response = await infrastructureApi.getServiceTasks(
				config.env,
				actualServiceName,
			);
	...
	const handleOpenSSH = async (task: ECSTaskInfo) => {
		try {
			...
			setActiveSSHSession({
				...
				containerName: isService
					? actualServiceName
					: `${config.project}_service_${config.env}`,
			});
		} catch (err) {
			setError(
				err instanceof Error ? err.message : "Failed to check SSH capability",
			);
		} finally {
			setCheckingSSH(null);
		}
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Enable secure remote access to {actualServiceName} containers for
						debugging
					</CardDescription>
				</CardHeader>
							...
							<div className="flex items-start gap-2">
								...
								<div className="flex-1">
									...
									<p className="text-xs text-gray-300">
										ECS Exec is enabled for secure SSH access to your{" "}
										{actualServiceName} containers. No bastion host required.
									</p>
								</div>
							</div>
						</div>
			...
			{config.workload?.backend_remote_access !== false && (
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Get running tasks and connect to your {actualServiceName}{" "}
							containers
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
							...
							<div className="flex items-center justify-between mb-2">
								...
								<div className="flex items-center gap-2">
									...
									<Button
										...
										variant="outline"
										onClick={() =>
											copyCommand(generateSSHCommand(primaryTask?.taskArn))
										}
										className="text-xs"
									>
								...
								<div className="space-y-3">
									...
									{runningTasks.length > 3 && (
										<p className="text-xs text-gray-500 text-center">
											+ {runningTasks.length - 3} more tasks
										</p>
									)}
								</div>
							</div>
							...
							<div className="space-y-3">
								<div>
									...
									<p className="text-xs text-gray-400">
										Click the "Get Tasks" button above to automatically fetch
										running tasks and fill in the task ARN.
									</p>
								</div>
								...
								<div>
									...
									<div className="bg-gray-900 rounded p-2 font-mono text-xs text-gray-300 overflow-x-auto">
										<pre>
											aws ecs list-tasks --cluster {config.project}_cluster_
											{config.env} --service-name {actualServiceName} --region{" "}
											{config.region}
										</pre>
									</div>
									<p className="text-xs text-gray-400 mt-1">
										Then use one of the returned task ARNs in the SSH command.
									</p>
								</div>

								<div>
									<p className="text-xs text-gray-300 mb-2">
										Option 3: Get Task Details
									</p>
									<div className="bg-gray-900 rounded p-2 font-mono text-xs text-gray-300 overflow-x-auto">
										<pre>
											aws ecs describe-tasks --cluster {config.project}_cluster_
											{config.env} --tasks $(aws ecs list-tasks --cluster{" "}
											{config.project}_cluster_{config.env} --service-name{" "}
											{actualServiceName} --query 'taskArns[0]' --output text)
											--region {config.region}
										</pre>
									</div>
									<p className="text-xs text-gray-400 mt-1">
										This command gets details about the first running task.
									</p>
								</div>
							</div>
						</div>
						...
						<div className="bg-gray-800 rounded-lg p-4">
							...
							<ul className="text-xs text-gray-400 space-y-1">
								...
								<li>
									...
									<code className="text-gray-300">aws ssm install-plugin</code>
								</li>
								<li>
									• Your container must include{" "}
									<code className="text-gray-300">/bin/bash</code>
								</li>
							</ul>
						</div>
					</CardContent>
					...
					<CardContent>
						<div className="text-center py-8">
							...
							<p className="text-xs text-gray-500 mb-4">
								Enable remote access to SSH into your {actualServiceName}{" "}
								containers for debugging and troubleshooting.
							</p>
							<Button
