...
export function ServiceEnvironmentVariables({
	config,
	accountInfo,
	node,
	onConfigChange,
	...
	const preconfiguredEnvVars = [
		// PostgreSQL variables - always shown but marked if not enabled
		{
			...
			value: config.postgres?.enabled
				? `${config.project}-${config.env}-rds.${config.region}.rds.amazonaws.com`
				: "Not available (PostgreSQL disabled)",
		...
		{
			name: "SQS_QUEUE_URL",
			value: config.sqs?.enabled
				? `https://sqs.${config.region}.amazonaws.com/${accountInfo?.accountId || config.ecr_account_id || "<ACCOUNT_ID>"}/${config.project}-${config.env}-${config.sqs.name || "queue"}`
				: "Not available (SQS disabled)",
		...
		// X-Ray variable - always shown but marked if not enabled
		{
			...
			value: serviceConfig.xray_enabled
				? "localhost:2000"
				: "Not available (X-Ray disabled)",
		...
		<div className="space-y-4">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Variables automatically set for all services
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="space-y-2">
						{preconfiguredEnvVars.map((envVar) => (
							<div
								key={envVar.name}
								className={`p-2 rounded-lg ${envVar.enabled ? "bg-gray-800" : "bg-gray-800/50"}`}
							>
								<code
									className={`text-xs font-mono ${envVar.enabled ? "text-blue-400" : "text-gray-500"}`}
								>
									{envVar.name}
								</code>
								<div
									className={`text-xs font-mono break-all mt-1 ${envVar.enabled ? "text-gray-300" : "text-gray-500"}`}
								>
									{envVar.value}
								</div>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Add your own environment variables for {serviceName}
					</CardDescription>
				</CardHeader>
				<CardContent>
									...
									<div className="mt-1">
										...
										<div className="flex items-center gap-1 mt-1">
											<Button
												...
												variant="ghost"
												onClick={() =>
													handleEditCustomVar(name, editingVarValue)
												}
												className="h-5 w-5 p-0"
											>
					...
					<Alert className="mt-4">
						...
						<AlertDescription className="text-xs">
							These variables are defined in the YAML configuration. For
							secrets, use Parameter Store at{" "}
							<code>{parameterStorePath}/*</code>
						</AlertDescription>
					</Alert>
				</CardContent>
			</Card>
