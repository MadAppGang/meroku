...
#      modules/workloads' and not modules/event_bridge_task's. It is the case
#      with no YAML edit available, so a wrong pointer here leaves the reader
#      nowhere.
...
# is the one modules/workloads and modules/event_bridge_task already have: that
# file owns the TEXT, byte for byte, against synthetic inputs, in a module with
# no provider. This file owns the WIRING — that the names this module hands over
# are the ones ECS will compare. Most of all it owns the upper-casing: an SSM
# parameter's last segment becomes a secret named `upper(...)`, so
# ".../scan_cursor_store" is the same name as the YAML variable
# SCAN_CURSOR_STORE, and a check comparing raw parameter paths would find
# nothing wrong with the configuration that fails.
#
# ---------------------------------------------------------------------------
# Every run here PLANS except the first, and as in modules/event_bridge_task
# that is a convenience rather than a constraint.
#
# In modules/workloads the SSM data sources `depends_on` the aws_ssm_parameter
# resources, so against an empty state the reads are deferred, the secret names
# are unknown, the condition is unknown — and Terraform POSTPONES an unknown
# precondition rather than failing it, which makes `expect_failures` unusable
# until an apply has put the parameters in state.
#
...
# ---------------------------------------------------------------------------
#
# Run: terraform test  (from modules/ecs_task)
...
mock_provider "aws" {
  ...
  # aws_scheduler_schedule reads task_definition_arn off this resource, and the
  # scheduler API takes an ARN without a revision — hence both attributes here,
  # not just `arn` as in modules/event_bridge_task, which has no scheduler.
  mock_resource "aws_ecs_task_definition" {
    defaults = {
      arn                  = "arn:aws:ecs:us-east-1:000000000000:task-definition/mock:1"
      arn_without_revision = "arn:aws:ecs:us-east-1:000000000000:task-definition/mock"
    }
  }
}
...
run "a_clean_configuration_applies_and_the_message_renders" {
  ...
  override_data {
    ...
    values = {
      names = ["/dev/acme/task/cleanup/env", "/dev/acme/task/cleanup/database_password"]
    }
  }

  # The rendered `environment` array, in order, as it reaches the container
  # definition — read through the LOWER-CASE key, which is the second thing this
  # assertion pins.
  #
  # That key was spelled `Environment` until the commit that added this file,
  # and the two spellings reach ECS identically: awstypes.ContainerDefinition
  # has no JSON struct tags, so the provider's encoding/json decode is
  # case-insensitive and re-serialises the normalised lower-case form. The
  # rename was verified a no-op against provider 5.100.0 — both spellings plan
  # to the identical state string and either one plans clean against a state
  # holding it.
  #
  # Read this assertion knowing it CANNOT see that. `mock_provider` never calls
  # the real provider, so the container_definitions read below is the raw
  # jsonencode output with the provider's normalisation not applied. Reverting
  # to `Environment` fails this run, and that failure means only "the key in
  # main.tf is not spelled `environment`" — not "the deploy is broken". The
  # genuine typo `Enviroment` fails it identically, and THAT one does break the
  # deploy: it yields no `environment` key at all, silently, with no error from
  # AWS. Telling those two apart needs the real provider, which is where they
  # were told apart.
  #
  # Order is load-bearing too: it goes through jsonencode into
...
# then creates the SSM parameter /dev/acme/task/cleanup/scan_cursor_store — note
...
run "a_declared_name_that_ssm_also_supplies_fails_the_plan" {
  ...
  override_data {
    target = data.aws_ssm_parameters_by_path.task
    values = {
      names = ["/dev/acme/task/cleanup/env", "/dev/acme/task/cleanup/scan_cursor_store"]
    }
  }

  # THE assertion. Not on the schedule, not on the schedule group and not on a
  # module output: on the resource whose RegisterTaskDefinition call AWS
  # rejects, which is the address Terraform prints next to the message.
  expect_failures = [
  ...
  assert {
    condition     = strcontains(module.task_env_secret_check.message["task"], "/dev/acme/task/cleanup") && strcontains(module.task_env_secret_check.message["task"], "environment_variables in project/dev.yaml")
...
#
# AWS_REGION is set by env.tf for every scheduled task, whether the user asked
# for it or not. It appears in project/<env>.yaml nowhere, so "remove it from
...
# reading the file that sets it. That file is modules/ecs_task/env.tf.
#
# Two wrong answers are available and both are excluded below.
# modules/workloads/env_services.tf sets no variable on this container at all.
# modules/event_bridge_task/env.tf is the near-twin — same seven names, same SSM
# prefix — and is still the wrong file: its EVENT_SOURCE is
# "<project>.event.<task>" where this module's is "<project>.task.<task>", so a
# reader sent there is reading a list that does not govern their task.
run "a_variable_the_module_sets_itself_collides_and_names_this_modules_env_tf" {
  ...
  override_data {
    target = data.aws_ssm_parameters_by_path.task
    values = {
      names = ["/dev/acme/task/cleanup/env", "/dev/acme/task/cleanup/aws_region"]
    }
  }
  ...
  assert {
    condition     = strcontains(module.task_env_secret_check.message["task"], "the rest of the list in modules/ecs_task/env.tf")
  ...
  assert {
    condition     = !strcontains(module.task_env_secret_check.message["task"], "modules/workloads") && !strcontains(module.task_env_secret_check.message["task"], "modules/event_bridge_task")
    error_message = "A scheduled task's reader has no modules/workloads/env_services.tf, and modules/event_bridge_task/env.tf is a different task's list that merely looks the same. Sending them to either at the one moment they have no other option is worse than saying nothing. Got: ${module.task_env_secret_check.message["task"]}"
  }
}
...
# its LAST segment, not its path relative to the prefix. /…/cleanup/db/api_key
...
run "a_nested_parameter_collides_on_its_last_segment" {
  ...
  override_data {
    target = data.aws_ssm_parameters_by_path.task
    values = {
      names = ["/dev/acme/task/cleanup/env", "/dev/acme/task/cleanup/db/api_key"]
    }
  }
  ...
  assert {
    ...
    error_message = "The read is recursive, so /dev/acme/task/cleanup/db/api_key is one of this container's secrets and its name is API_KEY. Got ${jsonencode(module.task_env_secret_check.collisions["task"])}."
  }
}
