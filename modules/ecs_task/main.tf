...
resource "aws_scheduler_schedule" "scheduler" {
  ...
  target {
    ...
    # Opt-in only. Omitted when unset so AWS keeps its own default (185); emitting a
    # bare default here would cut every existing task's retry budget on the next apply.
