...
// handleS3Event processes S3 object change events
func (h *EventHandler) handleS3Event(ctx context.Context, event events.CloudWatchEvent) (string, error) {
	...
	// Find services that use this env file
	affectedServices := h.findServicesForS3File(bucketName, objectKey)
	...
	for i, serviceName := range affectedServices {
...
// findServicesForS3File finds all services that use a specific S3 env file
func (h *EventHandler) findServicesForS3File(bucket, key string) []string {
	var services []string

	for serviceName, files := range h.config.ServiceConfig {
		for _, file := range files {
			if file.Bucket == bucket && file.Key == key {
				services = append(services, serviceName)
				break // Move to next service once we find a match
			}
		}
	}
...
// GetServiceConfigSummary returns a summary of S3 file to service mappings
func (h *EventHandler) GetServiceConfigSummary() map[string][]config.S3File {
	return h.config.ServiceConfig
}
